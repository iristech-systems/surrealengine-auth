...
class SurrealEngineAuth:
    ...
    def register_user(cls, username: str, email: str, password: str, 
                     is_admin: bool = False, metadata: Dict[str, Any] = None) -> Tuple[UserBuiltin, bool]:
        """
    ...
    @classmethod
    def authenticate(cls, username_or_email: str, password: str) -> Optional[UserBuiltin]:
        """
    ...
    @classmethod
    async def authenticate_async(cls, username_or_email: str, password: str) -> Optional[UserBuiltin]:
        """
    ...
    @classmethod
    def authenticate_with_api_key(cls, api_key: str) -> Optional[UserBuiltin]:
        """
        ...
        return cls._user_class.objects.get_sync(email=user[0].get('email'))

    @classmethod
    async def authenticate_with_api_key_async(cls, api_key: str) -> Optional[UserBuiltin]:
        """
        ...
        if not check_key or not check_key.is_active or not (user := await check_key.resolve_relation('user_keys')) or not len(user) > 0:
            return None

        return await cls._user_class.objects.get(email=user[0].get('email'))

    @staticmethod
    def create_api_key(user: UserBuiltin, name: str, expires_in_days: int = 365,
    ...
    @staticmethod
    async def create_api_key_async(user: UserBuiltin, name: str, expires_in_days: int = 365,
                      scopes: List[str] = None, metadata: Dict[str, Any] = None) -> Dict:
        """
        ...
        return await user.create_api_key_async(
            name=name,
            expires_in_days=expires_in_days,
            scopes=scopes,
            metadata=metadata
    ...
    @staticmethod
    def revoke_api_key(user: UserBuiltin, key_id: str) -> bool:
        """
    ...
    @staticmethod
    async def revoke_api_key_async(user: UserBuiltin, key_id: str) -> bool:
        """
        ...
        return await user.revoke_api_key_async(key_id)
    ...
    @staticmethod
    async def refresh_api_key_async(api_key: APIKey, expires_in_days: int = 365) -> APIKey:
        """
    ...
    @classmethod
    def get_user_by_id(cls, user_id: str) -> Optional[UserBuiltin]:
        """Get a user by ID (synchronous version)."""
        try:
            return cls._user_class.objects.get_sync(id=user_id)
        except:
            return None

    @classmethod
    async def get_user_by_id_async(cls, user_id: str) -> Optional[UserBuiltin]:
        """Get a user by ID (asynchronous version)."""
        try:
            return await cls._user_class.objects.get(id=user_id)
        except:
            return None

    @classmethod
    def get_user_by_username(cls, username: str) -> Optional[UserBuiltin]:
        """Get a user by username (synchronous version)."""
        return cls._user_class.objects.filter_sync(username=username).first_sync()

    @classmethod
    async def get_user_by_username_async(cls, username: str) -> Optional[UserBuiltin]:
        """Get a user by username (asynchronous version)."""
        return await (await cls._user_class.objects.filter(username=username)).first()

    @classmethod
    def get_user_by_email(cls, email: str) -> Optional[UserBuiltin]:
        """Get a user by email (synchronous version)."""
        return cls._user_class.objects.filter_sync(email=email).first_sync()

    @classmethod
    async def get_user_by_email_async(cls, email: str) -> Optional[UserBuiltin]:
        """Get a user by email (asynchronous version)."""
        return await (await cls._user_class.objects.filter(email=email)).first()
    ...
    @staticmethod
    async def get_api_key_async(key_id: str) -> Optional[APIKey]:
        """Get an API key by ID (asynchronous version)."""
        try:
            return await (await APIKey.objects.filter(key_id=key_id)).first()
        except:
            return None

    @classmethod
    def list_users(cls, limit: int = 100, offset: int = 0) -> List[UserBuiltin]:
        """List users with pagination (synchronous version)."""
        return cls._user_class.objects.filter_sync().limit(limit).start(offset).all_sync()

    @classmethod
    async def list_users_async(cls, limit: int = 100, offset: int = 0) -> List[UserBuiltin]:
        """List users with pagination (asynchronous version)."""
        return await cls._user_class.objects.filter().limit(limit).start(offset).all()
    ...
    async def list_api_keys_async(user: UserBuiltin) -> List[APIKey]:
        ...
        return await user.resolve_relation('user_keys')
    ...
    @staticmethod
    def update_user(user: UserBuiltin, **kwargs) -> UserBuiltin:
        """
    ...
    @staticmethod
    def deactivate_user(user: UserBuiltin) -> UserBuiltin:
        """
    ...
    @staticmethod
    def activate_user(user: UserBuiltin) -> UserBuiltin:
        """
